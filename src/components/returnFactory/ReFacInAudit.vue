<template>
  <div id="main">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item><a href="#">返厂入库单审核</a></el-breadcrumb-item>
    </el-breadcrumb>
    <hr>
    <!-- 首页表头 -->
    <el-form
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          type="primary"
          @click="rushenheRese('Factoryform')"
        >重置</el-button>
        <el-button
          type="primary"
    ...
    <el-pagination
      ...
      :total="page.total"
      style="margin-left: 36%;margin-top: 380px;"
    >
    </el-pagination>
...
export default {
  ...
  methods: {
    ...
    Exportdocument () {
      location.href = "http://localhost:8080/RFactoryshenhe/Export/";
    },
    // 首页搜索
    rushenhesearch () {

      this.$axios.post("/RFactoryshenhe/Rshenhesearch", this.Factoryform).then(response => {
    ...
    shenhe (row) {
      ...
      this.$axios.get("/RFactoryshenhe/RFshenhe?" + "pageNumber=" + this.shenhepage.pageNum + "&pageSize=" + this.shenhepage.pageSize + "&RFshenheid=" + this.number).then(response => {
    ...
    yijiansub (yijianform) {				//审核意见,状态
      ...
      this.$refs[yijianform].validate((valid) => {
        if (valid) {
          ...
          this.$axios.post("/RFactoryshenhe/yijiansub", this.yijianform).then(response => {
            if (response.data == 1) {
              this.$message({
                message: '操作成功',
                type: 'success'
    ...
    RFAudit (row) {
      ...
      this.$axios.get("/RFactoryshenhe/RFAudit?" + "RFshenheid=" + this.number).then(response => {
